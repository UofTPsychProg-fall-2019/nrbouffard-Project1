...
# Write out a csv with the counterbalance for altenrating scene and color blocks (scenecolor or color scene)
...
for c in np.arange(len(conds)): # Run this loop which creates the two possible counterbalance orders
    condOrders.append(conds)
    conds = conds[-1:] + conds[:-1]

# save out each order in two different files that will be used in the experiment 
